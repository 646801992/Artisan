...
public class ScriptSolverCompiler : IDisposable
{
    ...
    private void ThreadProc()
    {
        ...
        while (true)
        {
            ...
            if (!ok)
            {
                Svc.Log.Error($"[ArtisanScript] 无法将脚本从队列中取出进行编译，请向开发者报告此错误。");
                throw new Exception("无法将脚本从队列中取出进行编译");
            }
            ...
            try
            {
                ...
                if (type == null)
                    throw new Exception($"源文件 {script.SourcePath} 不包含任何派生自求解器的类。");
            ...
            {
                DuoLog.Error($"无法编译 {script.SourcePath}，请参阅日志了解详情。");
                Svc.Log.Error(ex, $"[ArtisanScript] 编译错误 {script.SourcePath}");
    ...
    private (byte[]? binary, string diagnostics, DiagnosticSeverity worst) Compile(string path, IImmutableList<MetadataReference> referenceList)
    {
        ...
        using (var peStream = new MemoryStream())
        {
            ...
            Svc.Log.Debug($"[ArtisanScript] 编译 {path} {(result.Success ? "无" : "有")} 错误:\n{diagnostics}");
    ...
    private static ImmutableList<MetadataReference> BuildReferenceList()
    {
        ...
        {
            Svc.Log.Verbose($"[ArtisanScript] 添加参考: {f}");
