...
namespace Artisan.UI
{
    ...
    unsafe internal class PluginUI : Window
    {
        ...
        public override void Draw()
        {
            ...
            if (ImGui.BeginTable($"ArtisanTableContainer", 2, ImGuiTableFlags.Resizable))
            {
                ...
                if (ImGui.BeginChild($"###ArtisanLeftSide", regionSize with { Y = topLeftSideHeight }, false, ImGuiWindowFlags.NoDecoration))
                {
                    ...
                    ImGui.Spacing();
                    if (ImGui.Selectable("持续模式", OpenWindow == OpenWindow.Endurance))
                    {
                    ...
                    ImGui.Spacing();
                    if (ImGui.Selectable("生成清单", OpenWindow == OpenWindow.SpecialList))
                    {
        ...
        {
            ImGui.TextWrapped($"在这里你可以调整Artisan的设置。其中的一些选项也能在制作期间调整。");
            ImGui.TextWrapped($"若要使用Artisan的手动模式高亮显示推荐技能, 请将您已解锁的所有制作技能放到可见的热键栏上。");
            ...
            if (ImGui.CollapsingHeader("模式选择"))
            {
                ...
                if (autoEnabled)
                {
                    ...
                    if (ImGui.SliderInt("设置延迟(ms)###ActionDelay", ref delay, 0, 1000))
                    {
                        if (delay < 0) delay = 0;
                ...
                if (Service.Configuration.UserMacros.Count > 0)
                {
                    ...
                    ImGuiComponents.HelpMarker($"使用宏来制作而不是由Artisan自动做决定。\r\n" +
            ...
            {
                if (ImGui.Checkbox("延迟获取下一步建议", ref delayRec))
                {
                ...
                ImGuiComponents.HelpMarker("若当前职业有专家认证, 将使用消耗“能工巧匠图纸”的技能。\n“设计变动”将会取代“观察”。");
                ...
                if (ImGui.Checkbox("当任务准备窗口弹出后让Artisan停止持续制作模式/暂停清单制作", ref requestStop))
                {
                ...
                if (requestStop)
                {
                    if (ImGui.Checkbox("当离开副本后让Artisan继续持续制作模式/继续清单制作", ref requestResume))
                    {
                    ...
                    if (requestResume)
                    {
                        if (ImGui.SliderInt("继续前等待时间（秒）", ref resumeDelay, 5, 60))
                        {
                ...
                if (ImGui.Checkbox("使用备用提升品质的循环 (需等级84+)", ref useAlternative))
                {
            ...
            {
                if (ImGui.Checkbox("禁用高亮推荐技能", ref disableGlow))
                {
                    Service.Configuration.DisableHighlightedAction = disableGlow;
                    Service.Configuration.Save();
                }
                ImGuiComponents.HelpMarker("如果你想手动制作, 该矩形高亮会向你指示推荐的技能。");

                if (ImGui.Checkbox($"禁用推荐技能弹出提示", ref disableToasts))
                {
                    Service.Configuration.DisableToasts = disableToasts;
                    Service.Configuration.Save();
                }

                ImGuiComponents.HelpMarker("当有推荐技能时会有弹出提示。");

                if (ImGui.Checkbox("禁用制作笔记迷你菜单", ref disableMini))
                {
                    Service.Configuration.DisableMiniMenu = disableMini;
                    Service.Configuration.Save();
                }
                ImGuiComponents.HelpMarker("在制作笔记内隐藏迷你菜单中的配置，但仍旧会显示宏菜单。");

                bool lockMini = Service.Configuration.LockMiniMenu;
                if (ImGui.Checkbox("保持迷你菜单吸附在游戏制作笔记窗口。", ref lockMini))
                {
            ...
            if (ImGui.CollapsingHeader("清单默认设置"))
            {
                ImGui.TextWrapped($"当创建制作清单时, 这些设置将会自动应用。");

                if (ImGui.Checkbox("跳过您已有足够数量的物品", ref Service.Configuration.DefaultListSkip))
                {
                    Service.Configuration.Save();
                }

                if (ImGui.Checkbox("自动精制魔晶石", ref Service.Configuration.DefaultListMateria))
                {
                    Service.Configuration.Save();
                }

                if (ImGui.Checkbox("自动修理", ref Service.Configuration.DefaultListRepair))
                {
                    Service.Configuration.Save();
                }
                ...
                if (ImGui.Checkbox("将添加到清单的新物品设置为简易制作", ref Service.Configuration.DefaultListQuickSynth))
                {
                    Service.Configuration.Save();
                }

                if (ImGui.Checkbox($@"当添加到清单后重置 “要添加的次数”。", ref Service.Configuration.ResetTimesToAdd))
                    Service.Configuration.Save();
            }
        }
    }
